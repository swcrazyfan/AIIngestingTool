...
echo "=== Testing Video Processing with Direct Curl ==="
...
echo "$curl_response" | grep -v "HTTP_STATUS_CODE:" | grep -v "TIME_TOTAL:" | jq . 2>/dev/null || echo "No valid JSON response or jq not available" 